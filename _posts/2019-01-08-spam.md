...
<img src="/images/spam/token-text.png" alt="Tokenized Text Messages">
...
Now we are ready to start building our feature matrix. Each row will represent a text message from our training set with zeros and ones corresponding to the character sequences for the given message. Our goal is to capture as much semantic meaning from the messages as possible while maintaining a scalable process with efficient use of resources.
